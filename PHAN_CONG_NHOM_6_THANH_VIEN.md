...
### 4.2. Thịnh - Tài Khoản + Điểm Thi + Điểm Cộng
Thịnh đã từng đảm nhận mảng này ở app trước, nên tiếp tục làm đầu mối, nhưng sẽ nhận thêm phần hỗ trợ kiểm tra và đồng bộ dữ liệu liên quan để phân việc đều hơn.
...
- Import điểm thi
- CRUD điểm cộng và quy ước cộng theo phương thức
...
- Hỗ trợ nhóm khi cần làm sạch CSDL để test lại end-to-end
...
- Huy tổng hợp các module để kiểm thử end-to-end
...
- `Thịnh`: tài khoản + điểm thi + điểm cộng + kiểm tra dữ liệu liên quan
- `Minh`: ngành tuyển sinh + chỉ tiêu
- `Khoa`: ngành - tổ hợp + bảng quy đổi
- `Khánh`: nguyện vọng và xét tuyển
- `Huy`: dashboard + app shell + theme + tích hợp
- `Huy`: dashboard + app shell + theme + tích hợp + tool hỗ trợ dữ liệu
